...
const TrackSlider: FunctionComponent<Props> = (props) => {
    ...
    function resizeResults() {
         ...
         */
        const slideWidth = window.innerWidth >= 1600 ? 4 : window.innerWidth < 1600 && window.innerWidth >= 1000 ? 3 : window.innerWidth < 1000 && window.innerWidth >= 650 ? 2 : 1;

        /**
         * Determining how much the sliders need to translate horizontally when a user clicks next on the slider.
         */
        const newTranslateAmount = props.searchResults.length != 0 ? (100/props.searchResults.length) * slideWidth : 0;
        ...
        setResultDimensions({
            ...
            slideIndex: slideWidth != resultDimensions.slideWidth ? sliderIndexAdjust : resultDimensions.slideIndex
        });
    }
     ...
     */
    const translateAmount = props.searchResults.length != 0 ? (100/props.searchResults.length) * resultDimensions.slideWidth : 0;
                            ...
                            <div className="ResultsSlider" style={{
                                ...
                                {props.searchResults.map((searchResult, i) => {
                                    return <Fragment key={i}>
                                        <Track searchResult={searchResult} width={resultDimensions.width} isRecommend={props.name === 'Recommendation'} search={props.search}></Track>
                                    </Fragment>
                                })}
                            </div>
                        </div>
