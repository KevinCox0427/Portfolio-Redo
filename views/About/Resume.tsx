import React, { FunctionComponent } from "react";

/**
 * React component to render my resume.
 */
const Resume: FunctionComponent = () => {
    return <div className="Resume">
        <div className="Column">
            <div className="Section Description">
                <h3 className="Heading">Full-Stack Web Developer & Graphic Designer</h3>
                <div className="HighligherWrapper">
                    <svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 164.9 33.19"><path fill="#ffeeb2" d="m127.3,32.44c-.22-.14-.48-.43-.66-.4-3.39.53-6.8.66-10.24.47-5.17-.28-10.33-.54-15.5-.85-.25-.01-1.67.25-.62-.68-.26-.1-.54-.32-.79-.29-.5.05-1.31.12-1.42.36-.69,1.47-2.56,1.68-3.8,1.55-5.86-.6-11.79-.7-17.65-1.08-3.91-.26-8.15.33-12.17.04-6.83-.49-13.6.3-20.41,0-1.81-.08-3.66.63-5.51.79-2.11.18-4.27.3-6.3-.46-3.35-1.27-6.76-.27-10.16-.27-5.03,0-9.78,1.25-14.7,1.54-4.01.24-6.67-2.69-7.07-7.62C.12,22.87.1,20.18,0,17.49c-.04-1.27.58-2.52,1.93-2.98,4.38-1.48,2.03-4.17,1.9-6.35-.13-2.09-.29-4.05,2.51-5.03,1.12-.39,2.83-.26,3.77-.16,3.77.38,7.08-1.01,10.72-1.13,6.62-.23,13.2-.54,19.85-.54,3.86,0,7.75-1.56,11.82-1.26,3.29.25,6.54.91,9.83,1,6.5.18,13.03-.1,19.51.25,1.97.11,4.35,1.46,6.02.57,3-1.59,5.95.61,8.76-.46.29-.11.8.14,1.21.23-.36.07-.79.06-1.07.22-.78.45-2.73-.62-2.47,1.17-1.09-.09-2.38.08-3.23-.33-1.22-.59-2.2-.14-3.24.01-.6.09-1.13.47-1.69.71.08.06.15.17.24.17.82.04,2.11.34,2.38.05.68-.73,1.36-.17,1.97-.23.58-.06,1.22.35,1.84.36,1.14.03,2.46-.38,3.39-.05,2.69.96,4.77-1.4,7.32-.61,0-.13.06-.34-.01-.37-.42-.19-.88-.35-1.32-.51.83-.33,1.75-1.03,2.47-.91,3.61.6,7.22.21,10.82.24.1,0,.2.03.3.04-.14.07-.28.19-.43.19-1.21.03-2.42.06-3.63.05-.74,0-1.64-.43-1.82.65-.03.15-.96.37-1.38.28-.84-.18-.22-.46.06-.69,0,0-.15-.2-.24-.2-1.47-.06-2.95-.1-4.42-.15.12.24.33.47.36.71.15,1.83,2.01,1.45,3.31,1.47,4.88.05,9.77-.02,14.66-.06.17,0,.35-.16.52-.25-.46-.25-1.34-.08-1.31-.82.91.29,1.8.79,2.73.84,2.62.14,5.27.23,7.88.1,8.76-.46,17.5-1.12,26.27-1.47,1.7-.07.37-1.82,1.71-1.36,1.71.58,3.02,1.37,3.19,3.22.33,3.66.36,7.35,1.78,10.95.69,1.75-.8,3.95-2.58,4.68-3.52,1.45-1.12,3.34-1.24,4.97-.17,2.38.72,5-3.09,6.16-.57.17-1.3-.01-1.96.01-6.98.28-14.07.1-20.88,1.11-2.65.39-5.28-.78-7.67.44h0ZM79.91,3.45s.51-.1.6-.75c.1-.72-.64-.84-1.19-.81-3.61.23-7.21.5-10.81.8-.43.04-.84.32-1.25.49.48.06.95.16,1.43.18,1.21.04,2.42.05,3.64.06,2.29.01,4.58.02,7.59.03Zm-49.5.05c1.53.1,2.57-.55,3.54-1.32-.73-.02-1.51-.19-2.16-.03-1.01.25-1.93.72-2.89,1.09.5.09,1.01.17,1.51.26Zm14.89-1.23c-1.71-.27-2.16-.09-1.93.33.87,1.64,2.67-.21,3.82.58.07.05.39-.12.59-.18-1.2-.35-2.4-.7-2.48-.72ZM5.39,31.88c-.69-.6-1.3-1.12-1.91-1.65-.12.21-.39.57-.33.62.61.41,1.27.77,1.93,1.12.05.03.21-.06.31-.09ZM7.74,4.69c.2-.18.36-.33.53-.48-.14-.07-.34-.21-.39-.18-.23.12-.41.29-.61.45.12.06.24.11.48.21Z"/></svg>
                    <h2>Kevin Cox</h2>
                </div>
                <p>An ideas-driven designer and developer that uses a diverse skill set to supply any graphical or technical need. An advocate for requiring a fundamental understanding of project ideas, web technologies, and visual design to provide a wide array of user-oriented solutions. An independent, highly motivated worker that specializes in the scaling of a business’s visual identity, web applications, graphics, and technical stacks to align with their goals.</p>
            </div>
            <div className="Section Education">
                <h3 className="Heading">Education</h3>
                <div className="Degree">
                    <h4>BFA Graphic Design</h4>
                    <p>2018-22 | Binghamton University</p>
                </div>
            </div>
            <div className="Section Experience">
                <h3 className="Heading">Experience</h3>
                <div className="Job">
                    <h4>Lead Developer</h4>
                    <span>Red Barn Technology Group  |  Binghamton, NY  |  Jun. 2022 - Jun. 2023</span>
                    <ul>
                        <li>Designed, developed, and managed a team of 5 to serve and maintain 7 ongoing clients.</li>
                        <li>Led and developed a middleware server to integrate an open-source ERP software for their internal departments.</li>
                        <li>Maintained, supported and repaired previous client's projects, significantly increasing user engagement and traffic.</li>
                    </ul>
                </div>
                <div className="Job">
                    <h4>Web Developer Intern</h4>
                    <span>FPS Apparel  |  Chester, NY  |  Jun. 2021 - Aug. 2021</span>
                    <ul>
                        <li>Assessed and advised their technology stack for optimization and additions for internal operations.</li>
                        <li>Created company graphics and maintained their Shopify storefront.</li>
                        <li>Jump-started a new integration, Ziflow, for clients to provide feedback on top of their mocked-up apparel designs.</li>
                    </ul>
                </div>
                <div className="Job">
                    <h4>Full Stack Development and Graphic Design Freelancer</h4>
                    <span>Dream State Graphics  |  New York, NY  |  Jan. 2021 - Jun. 2023</span>
                    <ul>
                        <li>My own freelancing company that takes on any graphical or web-based projects.</li>
                        <li>Projects included a menu for a local diner, a website for a pool company to take inquiries, and a teaching application where teachers can create a “teaching map” to overlay a student commenting system.</li>
                    </ul>
                </div>
            </div>
        </div>
        <div className="Column">
            <div className="Section Info">
                <div className="ImageWrapper">
                    <div className="Outline"></div>
                    <img src="https://dreamstateospublic.s3.us-east-2.amazonaws.com/headshotBW.jpeg" loading="lazy"></img>
                </div>
                <div className="InfoWrapper">
                    <div className="Link">
                        <h3>Website</h3>
                        <a href="https://www.dreamstate.graphics">www.dreamstate.graphics</a>
                    </div>
                    <div className="Link">
                        <h3>Email</h3>
                        <a href="mailto:kevin@dreamstate.graphics">kevin@dreamstate.graphics</a>
                    </div>
                    <div className="Link">
                        <h3>Phone</h3>
                        <a href="tel:8456497476">(845) 649-7476</a>
                    </div>
                    <div className="Link">
                        <h3>Area</h3>
                        <p>New York</p>
                    </div>
                    <div className="Link">
                        <h3>LinkedIn</h3>
                        <a href="https://www.linkedin.com/in/kevincox0427/">linkedin.com/in/kevincox0427</a>
                    </div>
                </div>
            </div>
            <div className="Section Skills">
                <h3 className="Heading">Skills</h3>
                <div className="SkillsWrapper">
                    <div className="Skill">
                        <h4>Front-end Development</h4>
                        <p> React, Redux, Angular, Vue, TypeScript, SASS, HTML, CSS, JavaScript, HTMX, JQuery, Webpack, Vite, Unit Testing</p>
                    </div>
                    <div className="Skill">
                        <h4>Back-end Development</h4>
                        <p>C#, Java, Python, PHP, Node.js, .Net, Laravel, Flask, Express.js, Docker, AWS, MVC Model, GIT, VIM, WordPress, SSR</p>
                    </div>
                    <div className="Skill">
                        <h4>Graphic Design</h4>
                        <p>UI/UX Design, Print Design, Vector Graphics, Raster Graphics, Motion Graphics, Mock-ups, Photography, Illustrator, Photoshop, Indesign, After Effects, XD, Figma</p>
                    </div>
                    <div className="Skill">
                        <h4>API & Database Development</h4>
                        <p>SQL, NoSQL, MongoDB, MySQL, PostgreSQL, GraphQL, REST, Analytics, Query Optimization, ORM abstractions</p>
                    </div>
                    <div className="Skill">
                        <h4>Soft Skills</h4>
                        <p>Natural leader, Effective time/task management, Great communication, Converts ideas into projects, Gives accurate estimates</p>
                    </div>
                </div>
            </div>
        </div>
    </div>;
}

export default Resume;