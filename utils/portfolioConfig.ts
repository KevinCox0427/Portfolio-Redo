...
const portfolioConfig: PortfolioConfig[] = [
    {
        ...
        solution: [
            "Since the client wanted a radical shift in their front-end AND back-end, I took this opportunity to re-develop the entire project in PHP as far away from the Wordpress framework as possible. I also re-designed / mocked it up alongisde them to make sure they undestood and approved exactly what they were getting.", 
            "To reduce content sprawl and confusion, I consoldated all their digital operations into two pages, a \"Buy\" page, a map-based UI with 10 filtering options to seach for every listing, and \"Sold\" page, a standardized form to provide a single source of intake for new listings. To tackle the server-side, I immediatly removed all \"Plugins\" from the site, only migrated the SQL database responsible for their listings and agents, and cleaned up the schemas. Finally, I wrote an Object-Oriented controller to handle CRUD operations and the queries for the \"Buy\" page's filters, and opened up API endpoints to recieve the GET and POST requests for the whole site."
        ],
    ...
    },
    {
        ...
        solution: [
            "The first thing I did was make a staging site, migrated the build, deleted all the \"Plugins\" (except for WooCommerce at the client's request), and migrated only the necessary SQL databases. Then I cleaned up the schemas, and created Object-Oriented controllers for optimized queries, which immediatly improved the site's performance by ~500%.",
            "Then I re-designed and re-developed the entire front-end to be much more legible and consistant. I created some banner graphics as well to create an asthetically pleasing landing page. I developed a new configuration UI, for their products similar to something like PC Part Picker, which immediatly saw use after launch. Finally, I recreated their forms for their support and quotes, and opened up all the necessary endpoints to make it all function."
        ],
    ...
    },
    {
        ...
        solution: [
            "First, I had to learn R to decode all of these images into 2-D arrays since the C#, JS, and Python libraries I tried couldn't do so. Then, due to the variety of the maps, I had to use R to figure out what part of the globe each one covered, as well as the delta change of its indeces. Based on these values, I then calculated which sections of the matricies I needed to just cover Morocoo, cut them out, and stored of all this information as metadata to be used in the SQL queries.",
            "Cutting out Morocco not only saved a siginificant amount of space and runtime, but allowed me to use the same bounds for longitude and latitude across all the maps. Then, I inserted the data into an SQL database, one a table for each row. Then after that it was just a matter of wiring it all together within a Node server, and creating a basic front-end for accepting parameters and displaying results."
        ],
